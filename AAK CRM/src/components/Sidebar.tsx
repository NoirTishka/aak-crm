...
export function SideBar() {
  const { selected, setSelected, kursants, loadKursants } = useKursantContext();
    ...
    <div className="flex flex-col h-full w-full px-4 py-2 bg-blue-50">
      <div className="bg-white rounded-2xl shadow-lg flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4 flex-shrink-0">
          <h1 className="text-xl text-black font-bold mb-4">АAK CRM</h1>
          ...
          <div className="relative w-full">
            <input
              ...
              className="text-black border border-gray-300 rounded-t-md px-2 py-1 w-full focus:outline-none mb-2"
            />
            {isFocused && searchTerm && (
              <div className="text-black absolute top-full left-0 right-0 bg-white border border-t-0 border-gray-300 rounded-b-md shadow max-h-60 overflow-y-auto z-10">
          ...
          <button
            ...
            className="!bg-gray-500 hover:!bg-gray-600 text-white py-2 rounded text-sm w-full"
          >
            + Добавить курсанта
          </button>
        </div>
        ...
        <div className="overflow-y-auto h-full scroll">
          {kursants.map((k) => (
            ...
            >
              <div className="font-semibold text-sm text-gray-700">{k.fio}</div>
              <div className="text-xs text-gray-600">ИИН: {k.iin}</div>
              <div className="text-xs text-gray-600">{k.phone}</div>
              <div className="text-xs text-gray-500">
                Записан: {k.registeredDate}
              </div>
              <span className="mt-1 inline-block text-green-600 text-xs font-semibold">
                Активный
              </span>
            </div>
          ))}
